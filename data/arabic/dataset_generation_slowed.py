...
dir = 'D:\\PythonScripts\\new_study\\wav_files\\accents\\total_samples\\'
for folder in os.listdir(dir):
    folder_dir = dir + folder + "\\"
    for accent in os.listdir(folder_dir):
        ACCENT_DIR = folder_dir + accent + "\\"
        for word in os.listdir(ACCENT_DIR):
            WORD_DIR = ACCENT_DIR + word + "\\"
            for file in os.listdir(WORD_DIR):
                print(WORD_DIR + file)
...
DIR = "D:\\PythonScripts\\new_study\\wav_files\\accents-recovery"
liste = [x for x in os.listdir(DIR) if "accelerated" in x]
for file in liste:
    os.remove(DIR + "\\" + file)
...
##
list_accents = ['arabic','french','spanish']
list_words = ['ask','bring','things','please','meet']
